...
const LoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  //     if (user) {
  //       await AsyncStorage.setItem('loggedUser', JSON.stringify(user));
  //       navigation.replace('NotesList'); // ✅ replaces stack, prevents going back
  //     } else {
  //       Alert.alert('Error', 'Invalid username/email or password');
  //     }
  //   } catch (error) {
  ...
  try {
    ...
    // ✅ Case-insensitive comparison for username or email
    ...
    if (user) {
      await AsyncStorage.setItem('loggedUser', JSON.stringify(user));
      navigation.replace('NotesList'); // ✅ prevents going back
    } else {
      Alert.alert('Error', 'Invalid username/email or password');
    }
  } catch (error) {
